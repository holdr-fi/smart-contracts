...
export class AssetHelpers {
   ...
   */
  sortTokens(tokens: string[], ...others: BN[][]): [string[], ...BN[][]] {
    ...
    const transpose = transposeMatrix([erc20Tokens, ...others]) as [string, ...BN[]][];
    const sortedTranspose = transpose.sort(([tokenA], [tokenB]) => cmpTokens(tokenA, tokenB));
    const [sortedErc20s, ...sortedOthers] = transposeMatrix(sortedTranspose) as [string[], ...BN[][]];
