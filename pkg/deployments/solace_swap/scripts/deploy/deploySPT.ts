...
import { NewWeightedPoolParams } from '../../types';
...
export const deploySPT = async function deploySPT(
  swpToken: Contract,
  ethToken: Contract,
  poolFactory: Contract,
  vault: Contract
): Promise<void> {
  const contractName = 'SPT';

  // Early return if task already done.
  {
    const output = task.output({ ensure: false });
    if (output[contractName] !== undefined) {
      return;
    }
  }
